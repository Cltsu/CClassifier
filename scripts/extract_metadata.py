    ...
    for home, dirs, files in os.walk(project_path):
        for filename in files:
            if filename.endswith('conflict.java'):
                # print('processing' + os.path.join(home, filename))

                conflict_path = os.path.join(home, 'conflict.java')
                resolve_path = os.path.join(home, 'resolve.java')
                ...
                #     result = chardet.detect(content)
                #     encoding = result['encoding']
                # with open(resolve_path, 'r', encoding=encoding) as r:
                ...
                try:
                    ...
                    with open(conflict_path, 'rb') as f:
                        ...
                        result = chardet.detect(content)
                        encoding = result['encoding']
                    with open(conflict_path, 'r', encoding=encoding, errors='ignore') as m:
                        merged =  [line for line in m.readlines()]
                ...
                try:
                    ...
                    with open(resolve_path, 'rb') as f:
                        ...
                        result = chardet.detect(content)
                        encoding = result['encoding']
                    with open(resolve_path, 'r', encoding=encoding, errors='ignore') as r:
                        resolved =  [line for line in r.readlines()]

                cur_confs = {   
                        "conflicting_chunks": flatten_text(extract_metadata(merged, resolved)),
                        'filename': home,
                    }
                for conf in cur_confs['conflicting_chunks']:
                    if 'label' in conf.keys():
                        label_distribution[conf['label']] += 1
                    label_distribution[conf['res_state']] += 1
